// Flight GST tax invoice PDF — pdfkit, single page A4.
//
// Layout mirrors `services/bus/invoice-pdf.ts` so finance can audit
// both product lines with identical visual structure. The only flight
// flavour is the booking-reference footer line (`Booking #TBNG-FL-...`
// instead of `TBNG-BUS-...`) and the FlightInvoiceDoc input type.
//
// Indian tax invoices are audited by humans (CA / GST officer) who
// must be able to recompute every line. We prioritise legibility +
// auditable math over branding flourish.
//
//   1. Header strip — "TAX INVOICE" + invoice number/date
//   2. Bill-from + Bill-to two-column block
//   3. Line items table (Description, SAC, taxable, GST rate, GST amt, total)
//   4. Tax summary box (CGST/SGST or IGST + grand total)
//   5. Footer — booking reference
...
/** Format paise as `Rs 1,23,456.78` (Indian-style grouping). */
...
export async function renderFlightInvoicePdf(invoice: FlightInvoiceDoc): Promise<Buffer> {
  ...
  // CANCELLED watermark — applied diagonally if the invoice was voided.
  if (invoice.status === 'CANCELLED') {
    doc
      .save()
      .rotate(-30, { origin: [PAGE_WIDTH / 2, 400] })
      .font('Helvetica-Bold')
      .fontSize(80)
      .fillColor('#FECACA')
      .opacity(0.6)
      .text('CANCELLED', 80, 380, { align: 'center' })
      .opacity(1)
      .restore();
  }
...
function drawParty(
  ...
  width: number,
  label: string,
  party: FlightInvoiceDoc['billFrom'],
): void {
  doc
    .font('Helvetica-Bold')
    .fontSize(8)
...
function drawLineTable(doc: PDFKit.PDFDocument, y: number, invoice: FlightInvoiceDoc): void {
  const rowHeight = 22;
  const headers = [
    { label: 'Description', x: MARGIN, w: 240 },
    { label: 'SAC', x: MARGIN + 248, w: 60 },
  ...
  doc.rect(MARGIN, y, CONTENT_WIDTH, rowHeight).fillColor(COLORS.brandSoft).fill();
  doc.font('Helvetica-Bold').fontSize(9).fillColor(COLORS.brand);
  for (const h of headers) {
    const align = ['Description', 'SAC'].includes(h.label) ? 'left' : 'right';
...
function drawTaxSummary(doc: PDFKit.PDFDocument, y: number, invoice: FlightInvoiceDoc): void {
  const boxX = MARGIN + CONTENT_WIDTH - 220;
  const boxY = y;
  const boxWidth = 220;
  const lineHeight = 18;

  doc.rect(boxX, boxY, boxWidth, 6 * lineHeight + 8).fillColor('#F8FAFC').fill();
